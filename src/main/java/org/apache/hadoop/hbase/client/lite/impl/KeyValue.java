...
public class KeyValue implements Cell {
...
//  }
  ...
  public KeyValue(final byte[] row, final byte[] family,
      final byte[] qualifier, final long timestamp, Type type,
      final byte[] value) {
    this(row, 0, Bytes.len(row), family, 0, Bytes.len(family), qualifier, 0, Bytes.len(qualifier),
        timestamp, type, value, 0, Bytes.len(value));
  }
...
//      final Object value, final int voffset, int vlength, List<Tag> tags) {
      final Object value, final int voffset, int vlength, List<?> tags) {
